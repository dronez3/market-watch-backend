...
/**
 * Compare symbols (default SPY vs QQQ) across:
 * - probability_up (same model as /api/prob)
 * - 7-bar momentum
 * - volatility (ATR vs 30d avg)
 * - latest sentiment (last N hours)
 *
 * GET /api/compare_pair?symbols=SPY,QQQ&hours=72&horizon_days=5
 */
export default async function handler(req: Request) {
  ...
  for (const symbol of symbols) {
    ...
    const latest: any = latestArr && latestArr.length ? latestArr[0] : null;
    ...
    // 4) Latest sentiment (within hours)
    const now = new Date();
    ...
    const sentiment: number | null = ss && ss.length ? Number((ss[0] as any).score) : null;

    // 5) Probability model (same weights)
    let p = 0.5;
  ...
  // Sort best to worst by probability
