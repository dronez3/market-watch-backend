 ...
 * Example:
 * /api/daily_multi_upsert_link?token=WRITER_TOKEN&symbol=AAPL&d1=2025-08-13&c1=220 ...
 */
export default async function handler(req: Request) {
  const url = new URL(req.url);
  const token = url.searchParams.get("token") || "";
  const expected = process.env.WRITER_BEARER_TOKEN || "";
  if (!expected) return jsonResponse({ ok: false, error: "Server missing WRITER_BEARER_TOKEN" }, 500);
