...
/**
 * GET /api/action_portfolio_list?user=UUID
 * Proxies -> /api/portfolio_list_link with server-side token.
 */
export default async function handler(req: Request) {
  ...
  const tgt = `${origin}/api/portfolio_list_link?token=${encodeURIComponent(token)}&user=${encodeURIComponent(user)}`;
